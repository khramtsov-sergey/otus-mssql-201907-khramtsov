...
BEGIN
    ...
    BEGIN TRY
		...
		SELECT @UserID = UserID FROM dbo.Users WHERE LoginName = USER_NAME()
		IF @UserID IS NOT NULL
		BEGIN
		INSERT INTO dbo.Items (TypeID,EditedBy,HeaderID,SerialNumber,WarehouseID,PlaceID,IventoryNumber,ParentID,StatusID)
		VALUES(@pTypeID,USER_NAME(),@pHeaderID,@pSerialNumber,@pWarehouseID,@pPlaceID,@pIventoryItem,@pParentID,@pStatusID)
...
DECLARE @countCompany TINYINT
SELECT @countCompany = count(*)
FROM dbo.Companies
...
select   'OfficeName'+CONVERT(Varchar(5),ID) 
		,ABS(CHECKSUM(NEWID()) % @countCompany)+1
		,'SomeAddress'+CONVERT(Varchar(5),ID)
		from IDs;
...
insert into dbo.Employees(FirstName,LastName,CompanyID)
select   'FirtsName'+CONVERT(Varchar(5),ID) 
		,'LastName'+CONVERT(Varchar(5),ID)
		,ABS(CHECKSUM(NEWID()) % @countCompany)+1
		from IDs;

update e
SET Email = LEFT(FirstName,1)+'.'+LastName+'@'+c.Name+'.com'
	,OfficeID = (SELECT TOP(1) OfficeID FROM dbo.Offices o WHERE e.CompanyID = o.CompanyID ORDER BY NEWID())
FROM dbo.Employees AS e
	JOIN dbo.Companies c
	ON e.CompanyID = c.CompanyId

DECLARE @countOffices TINYINT
SELECT @countOffices = count(*)
FROM dbo.Offices
...
select   'Warehouse'+CONVERT(Varchar(5),ID) 
		,ABS(CHECKSUM(NEWID()) % @countOffices)+1
		from IDs;
...
,IDs(ID) as (select row_number() over (order by (select null)) from N5)
insert into dbo.Places(EmployeeID,OfficeID)
select   ABS(CHECKSUM(NEWID()) % @countEmployees)+1
		,ABS(CHECKSUM(NEWID()) % @countOffices)+1
		from IDs;
GO
