-- Выбираем из vwUserFileInActive информацию по пачке файлов, которые по сроку жизни и ID папки подлежат удалению
...
-- Проверяем какие файлы из выбранной пачки попадают под кастомные условия удаления (расширение, размер, регулярка по имени, владелец и т.д. )
